...
- **Aperant feature integration layer:** after the BMAD engine swap is complete, integrate the existing Aperant feature surfaces with BMAD instead of leaving them parallel. Preserve the existing features, but make them BMAD-aware:
  - **Roadmap → BMAD planning:** add "Create Product Brief" / "Create PRD" actions on roadmap items that launch `bmad-product-brief` or `bmad-create-prd` with the feature context preloaded.
  - **Ideation → BMAD story creation:** add "Convert to Story" on ideation findings; writes or seeds a BMAD story/epic artifact with acceptance criteria and references, then routes through `bmad-create-story` / `bmad-dev-story`.
  - **Changelog → BMAD outputs:** changelog generation reads `sprint-status.yaml`, completed story files, and `bmad-retrospective` outputs as primary sources, not legacy `.auto-claude/specs`.
  - **Context → `project-context.md`:** Aperant's Context UI becomes the editor/viewer for BMAD's `_bmad-output/project-context.md` / `docs/project-context.md` style assets, and can invoke `bmad-generate-project-context`.
  - **Insights → persona-aware exploration:** Insights can run free-form project exploration through BMAD personas (Mary for product/analysis, Winston for architecture, Amelia for implementation) without forcing a workflow.
  - **PR/GitHub features → BMAD validation:** PR review and issue import can offer "turn this into a BMAD story" and "run implementation readiness / code review" actions where appropriate.
- **Agent Terminal / IDE delegation backend:** preserve Agent Terminals as integrated PTY shells and make them BMAD-aware, but split "terminal/config UI" from "managed auth":
  - **Plain Terminal Mode (safe default):** spawn the user's normal shell in a PTY. Do not inject `CLAUDE_CONFIG_DIR`, `CLAUDE_CODE_OAUTH_TOKEN`, or app-managed Claude profile env. If the user runs `claude`, `codex`, or `cursor-agent`, the official CLI owns auth through the user's normal shell/filesystem state. BMad Studio may attach the terminal to a project/story/persona, paste user-approved commands/prompts, monitor stdout/stderr, and save terminal metadata.
  - **Configuration UI (safe and desirable):** expose/edit project-level MCP config, hooks, skills, rules, prompt templates, BMAD customizations, and terminal presets as filesystem-backed settings. This is just a UI over local config files and should be integrated deeply with BMad modules/workflows.
  - **Claude Code boundary:** do not reuse Claude Code Max OAuth in app-managed third-party flows. Existing Claude profile/env injection (`CLAUDE_CONFIG_DIR` or `CLAUDE_CODE_OAUTH_TOKEN`) must not be used for BMAD Agent Terminal execution. If retained for legacy compatibility, it must be clearly separate from the safe BMAD terminal path and disabled by default for Claude Max users.
  - **Codex Pro:** support via Codex CLI subprocess/PTY, or the existing Codex OAuth profile path only while it remains functional and policy-safe. Prefer the CLI boundary when in doubt.
  - **Cursor Agent Pro:** prefer Cursor Agent CLI via ACP over stdio (or PTY fallback). Cursor's API-key / ACP design is the cleanest sanctioned third-party integration path.
  - **UX goal:** Agent Terminals become optional BMAD execution/config surfaces, not the primary engine. The same BMAD workflow state still writes to `_bmad-output/`.
